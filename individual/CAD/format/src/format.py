...
def readfile(row):
    ...
    """
    logger.info(f"reading in dataset for {row.fileyear}")
...
def checkasserts(df, rules):
    ...
    for year in range(2019, 2025):
        assert year in df.fileyear.values
...
def readconcat(ref, rules):
    ...
    full = pd.concat(dfs)
    full.fileyear = full.fileyear.astype(int)
...
if __name__ == '__main__':
    ...
    for col in datecols:
        logger.info(f"formatted datetime col:\t{col}")
